#include "Library.h"
    ...
    int Library::getPoints(std::string word) {
        ...
        void Library::iniciar_arbol(std::string filename){
            if(original_ini){
                std::cout<<"\nEl árbol del diccionario ya ha sido inicializado\n";
                return;
            }
            std::ifstream file(filename);

            if(!file.is_open()){
                std::cout<<"\nEl archivo " << filename <<" no existe o no puede ser leído\n";
                return;
            }
            ...
            if(!original.isEmpty()){
                std::cout<<"\nEl árbol del diccionario se ha inicializado correctamente\n";
                original_ini = true;
            }
            file.close();
        }
        ...
        void Library::iniciar_arbol_reverso(std::string filename){
            if(reverse_ini){
                std::cout<<"\nEl árbol del diccionario inverso ya ha sido inicializado\n";
                return;
            }
            std::ifstream file(filename);
            ...
            if(!reverse.isEmpty()) {
                std::cout<<"\nEl árbol del diccionario inverso se ha inicializado correctamente\n";
                reverse_ini = true;
            }
            file.close();
        }
        ...
        void Library::prefijos(std::string Prefix){
            ...
            std::list<std::string> lista_prefijos = tempNode->data.lookPrefix(Prefix);  //inorder por detrás
            if(lista_prefijos.empty()){
                std::cout<<"\nPrefijo "<<Prefix<<" no pudo encontrarse en el diccionario\n";
                return;
            }
            std::cout<<"\nLas palabras que inician con este prefijo ("<< Prefix <<") son:\n\n";
        ...
        void Library::sufijos(std::string Sufix){
            ...
            std::list<std::string> lista_sufijos = tempNode->data.lookSufix(Sufix); //Inorder por detrás
            if(lista_sufijos.empty()){
                std::cout<<"\nSufijo "<<reverseWord(Sufix)<<" no pudo encontrarse en el diccionario\n";
                return;
            }
            std::cout<<"\nLas palabras que inician con este sufijo ("<< reverseWord(Sufix) <<") son: \n\n";
